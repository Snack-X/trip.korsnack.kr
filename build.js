...
const HTMLMinify = require("html-minifier").minify,
      HTML_MIN_OPT = { includeAutoGeneratedTags: false, collapseWhitespace: true },
...
async function build(basepath, outDir) {
  ...
  await copyFiles(basepath, outDir, travels);
}
...
async function prepareRenderer(basepath) {
  ...
  md.renderer.rules.image = function(tokens, idx, options, env, self) {
    ...
    if(reYoutube.test(token.attrs[aIndex][1])) {
      const id = token.attrs[aIndex][1].match(token)[1];
    ...
    return mdDefaultImage(tokens, idx, options, env, self);
  };
}
...
async function copyFiles(basepath, outDir, travels) {
  // Copy scripts
  try { await fs.mkdir(path.join(outDir, "assets/scripts")); } catch(err) { }
  await _copyFile(
    path.join(basepath, "_assets/scripts/script.js"),
    path.join(outDir, "assets/scripts/script.js")
  );
...
async function _attachExifData(outDir, html) {
  // Cheerio sucks at handling entities
  html = html.replace(/&([lg]t);/g, "&amp;$1;");
